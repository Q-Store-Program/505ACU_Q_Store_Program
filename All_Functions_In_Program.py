...
def updateRank(namesListListbox, newRankEntry):
...
def updateQuantity(storeID, newQuantityEntry):
...
def removeStoreOptions(itemQuantityEntry):
...
def addAccount(namesListListbox,confirmedPasswordEntry,secretQuestionEntry,secretQuestionAnswerEntry):
...
def questionChecker(adminAccountQuestionAnswer,adminAccountSecretQuestionAnswer,passwordWindow,adminAccountPassword):
...
def logIn():
    ...
    if userUsernameEntry not in usernames:
        ...
        passwordErrorLabel= ctk.CTkLabel(
            leftTopFrame,
            text= "Incorect Username. Return to log in page and try again.",
            font= standardFont
            )
        passwordErrorLabel.pack(pady= standardYPadding)
...
def getPassword(userNameEntry):
    ...
    if userUsernameEntry not in usernames:
        ...
        cursor = connection.cursor()
        data = cursor.execute(f"SELECT Secret_Question,Secret_Question_Answer,Password FROM Accounts WHERE Username = '{userUsernameEntry}'").fetchall()
        formatted_data = []
        for item in data:
            accountSecretQuestion = item[0]
            accountSecretQuestionAnswer = item[1]
            accountPassword = item[2]
            formatted_data.append(f"{accountSecretQuestion}, {accountSecretQuestionAnswer}, {accountPassword}")
